    ...
    <div class="item_product" @click.stop="gotoDetailed(product.sku)">
      ...
      <h1 class="item_product_title">
        ...
        <template v-if="product.ishaitao == '1'">
          <span class="haitao">【全球购】</span>
        </template>
        <span class="txt_title">{{product.title}}</span>
      </h1>
      <div class="item_product_price">
        ...
        <div class="item_product_price_vip" v-if="product.vip_price">
          ...
          <img
            ...
            alt
          />
          <!-- <template v-if="product.is_open_le_vip == 1">
                        <img style="width: 40rpx;height:24rpx;vertical-align: middle;" src="../../../../static/images/search/search_sign_le.png" alt="">
          </template>-->
        </div>
        <div
          class="item_product_tags"
          :class="{'active':initDetail == 'other'}"
          v-for="(item,index) in product.award_short_tag"
          :key="index"
        >
          <span>{{item}}</span>
        </div>
      ...
      <div class="item_product_footer">
        <div class="item_product_footer_number">
          ...
          <template v-if="product.prodcutsellamount > 10">
            <span>{{product.prodcutsellamount}}人已购</span>
          </template>
        </div>
  ...
  methods: {
    ...
    shanAddCart(product) {
      ...
      if (product.multi_sku == 1) {
        ...
        this.goodsSpecPrice = product.sale_price;
        this.goodsSpecTile = product.prod_title;
