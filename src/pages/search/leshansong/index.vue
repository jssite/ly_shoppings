<template>
    <div class="search">
            ...
            <div class="search_input">
                ...
                <div class="search_input_wrap">
                    <template v-if="input_value">
                        <span class="search_input_icon_clear" @click.stop="funcCLearKeysWords"><icon type="clear" size="18" color="#999999" /></span>
                    </template>
                    <input type="text" @focus="funcInputFocus" confirm-type="done" @confirm="funcSearchSubmit" v-model="input_value" class="search_input_wrap_value" :placeholder="placeholder" placeholder-style="color:#cacaca;">
                </div>
        ...
        <div class="search_dialog_lists" v-if="input_value && input_foucs">
            <div class="search_dialog_result" v-for="(item,index) in suggest" :key="index" @click="funcSearchKeysResult(item)">{{item}}</div>
        </div>
        <!-- End -->
        <div class="search_keys" v-if="input_value == ''">
            ...
            <div class="search_keys_section" v-if="history_words.length">
                ...
                <div class="search_keys_wrap">
                    <span class="search_key" v-for="(item,index) in history_words" :key="index" @click="funcSearchKeysResult(item)">
                        {{item}}
                    </span>
                </div>
            </div>
            ...
            <div class="search_keys_section">
                ...
                <div class="search_keys_wrap">
                    <span class="search_key" v-for="(item,index) in hot_words" :key="index" @click="funcSearchKeysResult(item)">
                        {{item}}
                    </span>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        funCartNum() {
            ...
            let shansongcartid = wx.getStorageSync('shansongcart_id');
            let params ={};
            if(tokenid){
                //获取购物车数量
                params = {
        ...
        funcSearchSubmit(e){
            ...
            if( this.input_value ){
                ...
                if( type == 1 ){
                    this.funcSearchKeysResult(content);
                } else if( type == 2 ){
                    wx.navigateTo({
                        url: '/pages/transferH5/main?url' + content
                    })
                }
            }
        },
