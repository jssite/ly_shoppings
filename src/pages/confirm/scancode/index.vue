<template>
  <div>
    <div class="backgroundView" :style="{'padding-bottom':global_margin_bottom + 'px'}">
      <div class="scanshop">
        <span style="font-size:30rpx;color:#151515;margin-left:30rpx">{{shop_name}}</span>
      </div>
      <order-header
      ...
      ></order-header>
      <div class="orderSource">乐商城</div>
      <div class="product" v-for="(item, index) in ly_sku_list" :key="index">
        <!-- <div class="productIcon"></div> -->
        <div class="icon_bg">
          <img
            class="product_icon"
            :src="(item.img && item.img != '')?item.img:'/static/images/twohr_seat_pic70x70.png'"
          />
...
export default {
  ...
  onLoad(options) {
    ...
    request.shops = shop_id;
    // if (options.shop_id) {
    //   request.shops = options.shop_id;
    // }
    // request.shops = 2847;
    if (options.skulist) {
      request.skulist = JSON.parse(options.skulist);
    }
  ...
  methods: {
    ...
    */
      ...
      getSmgSubmitOrder(this.request_info).then(response => {
        wx.hideLoading();
    ...
    funcFillNoramlData(res) {
      ...
      if (res.leyou_flash) {
        ...
        if (support && this.is_select_meituan) {
    ...
    funcSubmitOrder() {
      ...
      wx.showLoading({ title: "加载中...", mask: true });
      userSmgSubmitOrder(pay_orderInfo).then(response => {
