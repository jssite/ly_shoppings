<template>
  <div>
    <div class="backgroundView" :style="{'padding-bottom':global_margin_bottom + 'px'}">
      ...
      ></order-header>
      <div class="orderSource sea" v-if="order_type ===3">
        <span>全球购</span>
        <div style="color:#ff5000;font-size:22rpx;display:flex;align-items:center">
          <img
            src="/static/images/confirm/order_icon_no.png"
            style="width:20rpx;height:20rpx;margin-right:10rpx"
          />
          <span v-if="responseData.product_prompt_info">{{responseData.product_prompt_info}}</span>
          <span v-else>海淘商品不支持7天无理由退货，请注意确认</span>
        </div>
      </div>
      <div class="orderSource" v-else>乐商城</div>
      <div class="product" v-for="(item, index) in ly_sku_list" :key="index">
        <!-- <div class="productIcon"></div> -->
        <div class="icon_bg">
          <img class="product_icon" :src="item.img" />
        ...
        <div class="productContent">
          <div class="productName"><span v-if="item.product_type == 1 || item.product_type == 6" class="pro_sale_style">{{item.product_type == 1 ?'赠品':'换购'}}</span> <span>{{item.marketing_title}}</span></div>
      ...
      <div class="payView">
        ...
        <div @click="funcGoCoupon('0')" class="payViewContent">
          <div>优惠券</div>
          <div class="couponRight">
            <div v-if="coupon_state_color == 0">无可用</div>
            <div
              v-else-if="coupon_state_color == 1"
              style="color:#ff5000"
            >{{responseData.coupon_available}}张可用</div>
            <div v-else>
              <span style="color:#ff5000">{{responseData.coupon_available}}张可用</span>
              <span>¥{{responseData.coupons.value}}</span>
            </div>
            <img class="arrow" src="/static/goodsdetail/lecard_arrow_right.png" />
          </div>
        </div>
        <div @click="funcGoCoupon('1')" class="payViewContent" v-if="coupon_free_state.free_show">
          <div>包邮券</div>
          <div class="couponRight">
            <span v-if="coupon_free_state.free_color_red" style="color:#FF5000">已使用包邮券</span>
            <span v-else>{{responseData.free_freight_list.length}}张可用</span>
            <img class="arrow" src="/static/goodsdetail/lecard_arrow_right.png" />
          </div>
        </div>
        <div class="payViewContent" v-if="responseData.is_vip">
          <div>企业尊享价（建议开启）</div>
          <!-- <switch
            v-if="responseData.is_use_vip"
            class="paySwitch"
            checked="true"
            color="#ff5000"
            @change="funcVip"
          ></switch>-->
          <!-- <switch v-else class="paySwitch" checked="false" color="#ff5000" @change="funcVip"></switch> -->
          <switch
            class="paySwitch"
        ...
        <div class="payViewContent" v-if="cash_money_use">
          ...
          <switch checked="true" class="paySwitch" color="#ff5000" @change="funcAccountMoney"></switch>
        </div>
        ...
        <!-- 分享抵扣 -->
        <div class="payViewContent" v-if="responseData.share_earning_message">
          <div>{{responseData.share_earning_message}}</div>
          <switch :checked="true" type="switch" color="#ff5000" @change="funcShareEarning"></switch>
        </div>
        <!-- End -->
        <div class="payViewContent" v-if="responseData.gift_card_amount" @click="funcGoGiftcard">
          <div>礼金卡</div>
          <div class="couponRight">
            <div v-if="gift_card_use">支付{{responseData.gift_card_amount}}元</div>
            <div v-else>使用礼金卡支付</div>
            <img class="arrow" src="/static/goodsdetail/lecard_arrow_right.png" />
          </div>
        </div>
        <div v-if="order_type != 3" class="payViewContent" @click="funcGoInvoice">
          <div>发票</div>
          <div class="couponRight">
        ...
        <div v-else class="payViewContent">
          <div>发票</div>
          <div class="couponRight">
            <div style="color:#999">海淘商品不支持开具发票</div>
          </div>
        </div>
      </div>
      <div class="leVipBgView" v-if="responseData.vip_info">
        <div class="leVipLeft">
          <div class="leVipLeftTop">
            <img
              src="/static/images/confirm/order_vip_icon_le.png"
              style="width:110rpx;height:52rpx;"
            />
            <div style="margin-left:10rpx; font-size:20rpx;">
              <!-- <wxParse :content="vip_info.order_discount"/> -->
              <wxParse :content="vip_info_title" />
            </div>
          </div>
          <!-- <wxParse :content="vip_info.vip_display_info"/> -->
          <wxParse :content="vip_info_content" />
        </div>

        <switch color="#ff5000" @change="funcOpenVip"></switch>
      </div>
    ...
    <div v-if="order_type == 2 || is_select_meituan" class="remindPopView">
    ...
    <div class="toast" @click="funcClose" v-if="show_toast" catchtouchmove="ture">
      ...
      >
        ...
        <div class="toast_button_bg">
          <button class="toast_go_pay" @click.stop="funcGoPayOrder(1)">返回购物车</button>
          <div
            style="text-align:center; margin-top:28rpx;color:#999;font-size:26rpx;"
          >我们将为您忽略以上商品继续下单</div>
        </div>
      ...
      <div class="toast_content_express" v-else-if="order_toast_type == 4">
        ...
        >
          ...
          <!-- <div class="toast_express_bottom">
            <div class="toast_express_bottom_left">
              <img style="width:20rpx;height:20rpx;background:red" src alt>
              <span style="font-size:26rpx;color:#999">点此查看运费详情</span>
            </div>
            <img style="width:20rpx;height:20rpx;background:red" src alt>
          </div>-->
        </div>
...
export default {
  ...
  mounted() {
    ...
    // 礼金卡
    EventBus.$on("giftcardSubmit", res => {
      this.request_info.card_num = res.card_num;
      this.card_price = res.card_price;
      if (res.card_pwd) {
        this.request_info.card_password = res.card_pwd;
        this.request_info.use_gift_card = "0";
    ...
    EventBus.$on("addressSubmit", info => {
      if (this.order_type == 3) {
        this.sea_resquest_info.address_id = info.address_id;
        this.haiTaoWorkInfo();
      } else {
    ...
    //  优惠券
    EventBus.$on("couponClick", ({ type, info }) => {
      if (this.order_type == 3) {
        if (type.is_coupon == "0") {
          // 优惠券
          if (type.is_use == "0") {
            // 使用
            this.sea_resquest_info.use_coupon = "0";
            this.sea_resquest_info.coupon_id = info.coupon_id;
          } else {
            // 不使用
            this.sea_resquest_info.use_coupon = "1";
            this.sea_resquest_info.coupon_id = "";
          }
        }
        this.haiTaoWorkInfo();
      } else {
        if (type.is_coupon == "0") {
          ...
          if (type.is_use == "0") {
            ...
            this.request_info.coupon_id = info.coupon_id;
            if (info.exclusion) {
              this.request_info.promotion = false;
            } else {
              this.request_info.promotion = true;
            }
          } else {
            // 不使用
          ...
          if (type.is_use == "0") {
            ...
            if (this.order_type == 6) {
              this.request_info.use_coupon_list = [info, this.exchangeCoupon];
            } else {
              this.request_info.use_coupon_list = [info];
            }
            // 使用
          } else {
            // 不使用
            ...
            if (this.order_type == 6) {
              this.request_info.use_coupon_list = [this.exchangeCoupon];
            } else {
  ...
  onLoad(options) {
    ...
    this.sa_page(this,false,'提交订单','');
    this.clearGlobalData();
    if (options.order_type) {
      this.order_type = parseInt(options.order_type);
    ...
    if (this.order_type === 0) {
      ...
      let request = {
        ...
        is_use_le_vip: true,
        use_cash_money: "0",
        is_ignore_gift_stock: false,
        promotion:true
      };
      if (options.skulist) {
        request.skulist = JSON.parse(decodeURIComponent(options.skulist));
      }
      if (options.igone_shoppingcart == '1') {
        request.igone_shoppingcart = true;
      }
      ...
      // 门店id
      if (options.shop_id) {
        request.shops = options.shop_id;
      }
      this.request_info = request;
    } else if (this.order_type === 3) {
      this.sea_resquest_info = {
        use_coupon: "0",
        is_use_first_fee: true,
        ship_method: "j"
      };
      ...
      this.exchangeCoupon = this.couponInfoWithMyCouponInfo(sender.coupon);
      let request = {
        use_free_freight: "0",
        is_o2o: "0",
        is_use_first_fee: true,
        ...
        use_cash_money: "0",
        is_use_share_earning: true,
        is_use_vip: sender.is_use_vip,
        sku_infos: sender.sku_info,
        use_coupon_list: [this.exchangeCoupon],
        is_use_le_vip: true,
        shop_lightning_order: 0
      };
      this.request_info = request;
      ...
      // 优惠券列表
      // this.use_coupon_list = []
      this.is_use_le_vip = false;
    }
    this.funcGetNetWorkInfo();
  },
  ...
  methods: {
    ...
    haiTaoWorkInfo() {
      ...
      getHaiTaoSubmitOrderInfo(this.sea_resquest_info).then(response => {
        wx.hideLoading();
        let code = response.header.res_code;
        if (code == 0) {
          let res = response.body;

          // if (res.consignee_id_card) {
          //   this.consignee_id_card = res.consignee_id_card;
          //   this.func_encry_id_num();
          // } else {
          //   this.encry_id_num = '';
          // }
          this.funcFillSeaData(res);
        } else if (
          code == 6001009 ||
          code == 6001008 ||
          code == 6001006 ||
    ...
    funcFillSeaData(res) {
      ...
      if (res.coupon_available && parseInt(res.coupon_available) > 0) {
        if (res.coupons && parseInt(res.coupons.value) > 0) {
          ...
          this.sea_resquest_info.use_coupon = "0";
          this.sea_resquest_info.coupon_id = res.coupons.coupon_id;
        } else {
      ...
      if (res.lately_address_id) {
        this.sea_resquest_info.address_id = res.lately_address_id;
      }
      let sku_list_temp = [];
    ...
    funcSeaSubmitOrder() {
      ...
      pay_orderInfo.address_id = this.sea_resquest_info.address_id;
      ...
      if (
        ...
        pay_orderInfo.coupon_id = this.sea_resquest_info.coupon_id;
      }
      pay_orderInfo.is_use_first_fee = this.sea_resquest_info.is_use_first_fee;
      wx.showLoading({ title: "加载中...", mask: true });
      userHaiTaoSubmitOrder(pay_orderInfo).then(response => {
        if (response.header.res_code == 0) {
          let res = response.body;
          this.funcSaOrder(res, 'mp_submitOrder', false);
          this.funcSaOrderDetail(res, 'mp_submitOrderDetail', false);
    ...
    */
      let self = this;
      wx.showLoading({ title: "加载中...", mask: true });
      getSubmitOrderInfo(this.request_info).then(response => {
        wx.hideLoading();
        let code = response.header.res_code;
    ...
    show_toast_info(res) {
      let allPro = res.ly.skulist;
      let errorPro = res.error_product_list;
    ...
    funcGoPayOrder(index) {
      if (index == 0) {
        // 缺货继续下单
        this.request_info.skulist = this.goods_enough_list;
        this.funcGetNetWorkInfo();
      } else {
        wx.navigateBack({
          delta: 1
    ...
    funcFillNoramlData(res) {
      ...
      // 普通订单 闪送头的显示
      if (this.order_type == 0 && res.leyou_flash) {
        let support = res.leyou_flash.support;
        if (support) {
          this.funcConfigFlashSelect();
          if (!this.is_auto_select_meituan) {
            this.is_auto_select_meituan = true;
      ...
      // 优惠券
      if (
        res.coupon_available &&
        parseInt(res.coupon_available) > 0 &&
        this.order_type != 6
      ) {
        if (res.coupons && parseInt(res.coupons.value) > 0) {
          this.coupon_state_color = 2;
    ...
    funcSubmitWithToast() {
      if (this.order_type == 3) {
        // 海淘订单验证身份证
        if (this.consignee_id_card && this.consignee_id_card != "") {
          this.funcSeaSubmitOrder();
        } else {
          this.funcToast("请输入正确的身份证号");
        }
        return;
      }
    ...
    funcSubmitOrder() {
      ...
      if (this.order_type === 3) {
        ...
        if (this.request_info.use_coupon == "0" && this.order_type != 6) {
          ...
          pay_orderInfo.promotion = this.request_info.promotion;
        }
        ...
        if (this.order_type === 1) {
          ...
          pay_orderInfo.shops = this.request_info.shops;
        } else if (this.order_type === 2) {
          this.funcOperationFlashData(pay_orderInfo);
        } else {
          if (this.order_type === 0 && this.is_select_meituan) {
            this.funcOperationFlashData(pay_orderInfo);
          } else {
            pay_orderInfo.ship_method = "j";
        ...
        if (this.order_type === 6) {
          pay_orderInfo.sku_infos = this.request_info.sku_infos;
        }
        if (this.request_info.igone_shoppingcart) {
          pay_orderInfo.igone_shoppingcart = this.request_info.igone_shoppingcart;
        }
        pay_orderInfo.is_use_share_earning = this.request_info.is_use_share_earning;
        wx.showLoading({ 
            title: "加载中...", 
            mask: true 
        });
        userSubmitOrder(pay_orderInfo).then(response => {
          if (response.header.res_code == 0) {
            let res = response.body;
            this.funcSaOrder(res, "mp_submitOrder", false);
            this.funcSaOrderDetail(res, "mp_submitOrderDetail", false);
            if (res.is_to_pay) {
              ...
              this.funcSaOrder(res, "mp_payOrder", false);
              this.funcSaOrderDetail(res, "mp_payOrderDetail", false);
            }
            ...
            wx.showModal({
              ...
              success(res) {
                if (res.confirm) {
                  ...
                  //  let addressStr = JSON.stringify(self.confirm_address_info);
                  //  let url = '/pages/shipAddress/changeAddress/main?info=' + addressStr + "&confirm=1";
                  // let url = '/pages/shipAddress/changeAddress/main?&confirm=1';
                  //  wx.navigateTo({
                  //    url: url
                  //  });
                }
    ...
    //使用分享赚抵扣
    funcShareEarning(e){
        this.request_info.is_use_share_earning = e.target.value;
        this.funcGetNetWorkInfo();
    },
    // 开通乐享卡
    funcOpenVip(e) {
        ...
        requestPayment({
          ...
          goodsList: res_pay_info.config.goodsList,
             ...
             success:function(res_wx_info){
                ...
                self.funcSaOrder(le_order_info, "mp_payOrder", true);
                self.funcSaOrderDetail(le_order_info, "mp_payOrderDetail", true);
             },
    ...
    funcGoFinshPage(options) {
        let params = encodeURIComponent(JSON.stringify(options));
        let url =
            "/pages/confirm/finish/main?pay_info=" + params
            "&is_le_vip=0";
        wx.redirectTo({
            url: url
        });
    ...
    funcSaOrder(res, sa_str, is_pay) {
      ...
      if (sa_str == "mp_payOrder") {
        if (is_pay) {
          mp_submit_order.paymentMethod = "微信";
        } else {
          mp_submit_order.paymentMethod = "余额";
        }
      }
      app.sensors.track(sa_str, mp_submit_order);
    },

    funcSaOrderDetail(res, sa_str, is_pay) {
      let orderType = "快递配送";
      if (this.order_type == 2 || this.is_select_meituan) {
        orderType = "闪送";
      } else if (this.order_type == 1) {
      ...
      this.ly_sku_list.map(pro_item => {
        let sa_pro = wx.getStorageSync("mp_" + pro_item.sku);
        ...
        app.sensors.track(sa_str, sa_pro);
        if (sa_str == "mp_payOrderDetail") {
          wx.removeStorageSync("mp_" + pro_item.sku);
        }
      });
    },
