...
sa.para.max_string_length = sa.para.max_string_length || 300;

if (typeof sa.para.send_timeout !== 'number') {
  sa.para.send_timeout = 1000;
}

sa.para.datasend_timeout = sa.para.datasend_timeout || 3000;

if (sa.para.batch_send) {
  if (typeof sa.para.batch_send !== 'object') {
    sa.para.batch_send = {};
  }
  sa.para.use_client_time = true;    
  sa.para.batch_send.send_timeout = sa.para.batch_send.send_timeout || 6000;
  sa.para.batch_send.max_length = sa.para.batch_send.max_length || 6;
}
...
_.getCurrentUrl = function(me){
  ...
  var query = '';
  if(_.isObject(me) && me.sensors_mp_url_query){
    query = me.sensors_mp_url_query;
  }
  if(path){
...
sa.sendStrategy = {
  ...
  batchWrite: function () {
    ...
    if (this.dataHasChange) {
      ...
      wx.setStorageSync('sensors_mp_prepare_data', sa.store.mem.mdata);
    }
  },
...
// 对所有提供的方法做代理暂存
_.each(['setProfile', 'setOnceProfile', 'track', 'register', 'clearAllRegister', 'quick'], function (method) {
  var temp = sa[method];
  sa[method] = function () {
...
sa.autoTrackCustom = {
  ...
  pageShow: function (not_use_auto_track) {
    var router = '系统没有取到值';
...
sa.quick = function () {
  ...
  } else if (arg0 === 'pageLoad') {
    if (arg1) {
      sa.autoTrackCustom.pageLoad(arg1);
    }
  } else if (arg0 === 'appHide' || arg0 === 'pageShow') {
    if (_.isObject(arg1)) {
      prop = arg1;
    }
    sa.autoTrackCustom[arg0](arg1, prop);
